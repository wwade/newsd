...
// int Group::FindArticleByMessageID(const char *find_messageid, unsigned long &articlenum)
		...
		bool force)
{
    ...
    {
        if ( strncmp(head[t].c_str(), "Newsgroups: ", strlen("Newsgroups: ")) == 0 )
	{
...
int Group::ParseArticle(string &msg, vector<string>&head, vector<string>&body)
{
	...
	if ( *ss == '\r' )				// end of line?
	{
	    ...
	    //     preserving the \r\n's within the line. 
	    //     Let's *not* unfold, to preserve line length enforced by browser.
	    //
	    //         in: "head1: abc\r\n<space>def\r\nhead2: 123\r\n"
...
void Group::UpdatePath(vector<string>&header)
{
        ...
        if ( strncasecmp(header[t].c_str(), pathstr, pathlen) == 0 )
	{
	    ...
	    if ( header[t].c_str()[5] != ' ' )
	        header[t].insert(5," ");		// "Path:lasthost.." -> "Path: lasthost.."
	    header[t].insert(6, info);			// "Path: lasthost.." -> "Path: thishost, lasthost.."
	    found = 1;
	}
    }
